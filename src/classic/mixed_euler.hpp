...
Mixed_Euler_Orientation(int n,
                        const vector<array<int, 3>> &edges,
                        Network_Flow<N, M> &flow,
                        int start = 0,
                        int finish = 0)
{
    ...
    for ( auto [u, v, type] : edges )
    {
        ...
        balance[u]++;
        balance[v]--;
        id.push_back(type == 0 ? flow.Add_Edge(u, v, 1) : -1);
    ...
    for ( int u = 1; u <= n; u++ )
    {
        ...
        if ( balance[u] > 0 )
        {
            flow.Add_Edge(source, u, balance[u] / 2);
            need += balance[u] / 2;
        }
        else if ( balance[u] < 0 )
            flow.Add_Edge(u, sink, -balance[u] / 2);
    }
    if ( flow.Max_Flow(source, sink) != need )
        return nullopt;
...
template <int N, int M>
optional<vector<pair<int, int>>>
Mixed_Euler_Trail(int n, const vector<array<int, 3>> &edges, Network_Flow<N, M> &flow)
{
    assert(n > 0);
    ...
    for ( int u = 1; u <= n; u++ )
        if ( degree[u] % 2 )
            odd.push_back(u);
    if ( odd.empty() )
        return Mixed_Euler_Orientation(n, edges, flow);
    if ( odd.size() != 2 )
        return nullopt;
    auto answer = Mixed_Euler_Orientation(n, edges, flow, odd[0], odd[1]);
    if ( answer )
        return answer;
    return Mixed_Euler_Orientation(n, edges, flow, odd[1], odd[0]);
}

...
struct Prime_Power_Roots
{
    ...
    ll Get(ll i, ll j, ll lift) const
    {
        ...
        ll x = (__int128)first * Number_Theory::Power(ratio[0], i, step) % step;
        x = (__int128)x * Number_Theory::Power(ratio[1], j, step) % step;
        return x + lift * step;
    }

    // p prime, 1 <= e, p^e <= 1e12, k > 0.
    // For odd p, g must be a primitive root modulo p^e. For p=2, g is unused.
    static optional<Prime_Power_Roots>
    Solve(ll a, unsigned long long k, ll p, int e, ll g = 0)
    {
        ...
        {
            ll logarithm = Discrete_Log::Solve(base, target, q);
            if ( logarithm < 0 )
                return false;
            ...
            if ( x < 0 )
                return false;
            answer.first =
                (__int128)answer.first * Number_Theory::Power(base, x, q) % q;
            answer.ratio[slot] = Number_Theory::Power(base, period, q);

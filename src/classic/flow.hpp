...
template<int N, int M> struct Bounded_Circulation
{
    ...
    bool Solve ()
    {
        ...
        {
            if ( balance[u] > 0 )
            { graph.Add_Edge(n + 1, u, balance[u]); need += balance[u]; }
            else if ( balance[u] < 0 ) graph.Add_Edge(u, n + 2, -balance[u]);
        }

...
{
    using ll=long long;
    int n; Dinic g; vector<ll> balance,lower;
    vector<int> ids; bool solved=false,feasible=false;
    BoundedCirculation(int n):n(n),g(n+2),balance(n+1) {}
    int add(int u,int v,ll lo,ll hi)
    {
        assert(!solved&&0<=lo&&lo<=hi);
        int id=ids.size(); ids.push_back(g.add(u,v,hi-lo)); lower.push_back(lo);
        balance[u]-=lo; balance[v]+=lo; return id;
    }
    bool solve()
    {
        assert(!solved); solved=true; ll need=0;
        for (int u=1;u<=n;u++)
            if (balance[u]>0) { g.add(n+1,u,balance[u]); need+=balance[u]; }
            else if (balance[u]<0) g.add(u,n+2,-balance[u]);
        return feasible=(g.flow(n+1,n+2)==need);
    }
    ll used(int id) const
    { assert(feasible); return lower[id]+g.used(ids[id]); }
};

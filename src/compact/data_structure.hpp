...
struct DSU
{
    ...
    int find(int x)
    {
        while (x!=fa[x]) { fa[x]=fa[fa[x]]; x=fa[x]; }
        return x;
    }
    bool merge(int x,int y)
    {
        ...
        fa[y]=x; siz[x]+=siz[y]; return true;
    }
};
...
struct RollbackDSU
{
    ...
    bool merge(int x,int y)
    {
        ...
        if (x==y) return false;
        if (siz[x]<siz[y]) swap(x,y);
        his.push_back({y,siz[x]}); fa[y]=x; siz[x]+=siz[y];
        return true;
    }
    void rollback(int t)
    {
        ...
        while (snapshot()>t)
        {
            auto [y,s]=his.back(); his.pop_back();
            siz[fa[y]]=s; fa[y]=y;
        }
    }
};
...
template<class T=long long> struct Fenwick
{
    ...
    void add(int x,T v)
    { assert(x>=1); for (;x<=n;x+=x&-x) a[x]+=v; }
    T sum(int x) const
    { T ans=0; for (;x;x-=x&-x) ans+=a[x]; return ans; }
    T query(int l,int r) const { return sum(r)-sum(l-1); }
    // Nonnegative frequencies; returns n+1 if k exceeds total.
    int kth(T k) const
    {
        assert(k>0); int x=0,b=1;
        while (b<=n/2) b*=2;
        for (;b;b>>=1)
            if (x+b<=n&&a[x+b]<k) { k-=a[x+b]; x+=b; }
        return x+1;
    }
};
...
struct LazySeg
{
    int n; vector<long long> sum,tag;
    ...
    {
        int m=(l+r)/2;
        apply(p*2,l,m,tag[p]); apply(p*2+1,m+1,r,tag[p]);
        tag[p]=0;
    }
...
struct XorBasis
{
    ...
    { for (int i=63;i>=0;i--) x=max(x,x^a[i]); return x; }
};

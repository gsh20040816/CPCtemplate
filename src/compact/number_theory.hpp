...
struct NumberTheory
{
    ...
    static bool prime(ull n)
    {
        ...
        for (ull a:{2ULL,325ULL,9375ULL,28178ULL,450775ULL,9780504ULL,1795265022ULL})
        {
            ...
            bool ok=false;
            for (int j=1;j<s;j++) { x=mul(x,x,n); if (x==n-1) { ok=true; break; } }
            if (!ok) return false;
        }
        return true;
    }
    static i128 exgcd(i128 a,i128 b,i128 &x,i128 &y)
    {
        ...
        i128 d=exgcd(b,a%b,y,x); y-=a/b*x; return d;
    }
    static ll inverse(ll a,ll m)
    {
        assert(m>0); a%=m; if (a<0) a+=m;
        i128 x,y; if (exgcd(a,m,x,y)!=1) return -1;
        return (x%m+m)%m;
    }
    // x = r (mod m); false means inconsistent. Throws if lcm exceeds int64.
    static bool crt(ll &r,ll &m,ll b,ll n)
    {
        assert(m>0&&n>0); r%=m; if (r<0) r+=m; b%=n; if (b<0) b+=n;
        i128 x,y; ll g=(ll)exgcd(m,n,x,y); i128 diff=i128(b)-r;
        if (diff%g) return false;
        i128 q=n/g,k=(diff/g*x%q+q)%q,mod=i128(m)*q;
        if (mod>LLONG_MAX) throw overflow_error("CRT modulus");
        r=(r+i128(m)*k)%mod; m=(ll)mod; return true;
    }
    // sum_{i=0}^{n-1} floor((a*i+b)/m), signed a,b; 0<=n<=1e9, 1<=m<=1e9.
    static i128 floor_sum(ll n,ll m,ll a,ll b)
    {
        assert(n>=0&&m>0); i128 ans=0;
        auto norm=[&](ll &v)
        { ll q=v/m; if (v%m<0) --q; v=(ll)(i128(v)-i128(q)*m); return q; };
        ans+=i128(norm(a))*n*(n-1)/2; ans+=i128(norm(b))*n;
        while (true)
        {
            ans+=i128(n)*(n-1)/2*(a/m); a%=m;
            ans+=i128(n)*(b/m); b%=m;
            i128 y=i128(a)*n+b;
            if (y<m) break;
...
struct PollardRho
{
    ...
    ull rho(ull n)
    {
        ...
        for (;;)
        {
            ...
            auto f=[&](ull v) { return (NumberTheory::mul(v,v,n)+u128(c))%n; };
    ...
    {
        if (n==1) return;
        if (NumberTheory::prime(n)) { a.push_back(n); return; }
        ull d=rho(n); split(d,a); split(n/d,a);
    }
    vector<ull> factor(ull n)
    { assert(n>=1); vector<ull> a; split(n,a); sort(a.begin(),a.end()); return a; }
};
...
struct LinearSieve
{
    ...
    LinearSieve(int n):lp(n+1),phi(n+1),mu(n+1)
    {
        ...
        for (int i=2;i<=n;i++)
        {
            if (!lp[i]) { lp[i]=i; prime.push_back(i); phi[i]=i-1; mu[i]=-1; }
            for (int p:prime)
            {
                if (p>n/i) break;
                int j=i*p; lp[j]=p;
                if (i%p==0) { phi[j]=phi[i]*p; mu[j]=0; break; }
                phi[j]=phi[i]*(p-1); mu[j]=-mu[i];
            }
        }
    }
};
...
template<int mod> struct ModInt
{
    ...
    { assert(e>=0); ModInt a=*this,r=1; for (;e;e>>=1,a=a*a) if (e&1) r=r*a; return r; }
    ModInt inv() const { assert(v); return pow(mod-2); } // prime modulus
    ModInt operator/(ModInt b) const { return *this*b.inv(); }
};

template<int mod> struct Binomial
{
    using Z=ModInt<mod>; vector<Z> fac,ifac;
    Binomial(int n):fac(n+1),ifac(n+1)
    {
        assert(0<=n&&n<mod); fac[0]=1;
        for (int i=1;i<=n;i++) fac[i]=fac[i-1]*i;
        ifac[n]=fac[n].inv();
        for (int i=n;i;i--) ifac[i-1]=ifac[i]*i;
    }
    Z choose(int n,int k) const
    { if (k<0||k>n) return 0; assert(n<(int)fac.size()); return fac[n]*ifac[k]*ifac[n-k]; }
};

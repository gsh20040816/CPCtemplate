...
{
    using G=IntegerGeometry; using P=G::Point; using I=G::I;
    // No pair exists for n<2. Exact squared distance, O(n log n).
    static optional<I> closest_pair(vector<P> p)
    {
        int n=p.size(); if (n<2) return nullopt;
        sort(p.begin(),p.end());
        for (int i=1;i<n;i++) if (p[i]==p[i-1]) return I(0);
        vector<P> tmp(n);
        ...
        function<I(int,int)> solve=[&](int l,int r)->I
        {
            ...
            {
                I best=I(1)<<120;
                for (int i=l;i<r;i++) for (int j=i+1;j<r;j++) best=min(best,G::dist2(p[i],p[j]));
                sort(p.begin()+l,p.begin()+r,by_y); return best;
            }
            int m=(l+r)/2; long long x=p[m].x;
            I best=min(solve(l,m),solve(m,r));
            merge(p.begin()+l,p.begin()+m,p.begin()+m,p.begin()+r,tmp.begin()+l,by_y);
            copy(tmp.begin()+l,tmp.begin()+r,p.begin()+l);
            vector<P> strip;
            for (int i=l;i<r;i++) if ((I(p[i].x)-x)*(I(p[i].x)-x)<best)
            {
                for (int j=(int)strip.size()-1;j>=0;j--)
                {
                    I dy=I(p[i].y)-strip[j].y; if (dy*dy>=best) break;
    ...
    static vector<P> minkowski(vector<P> a,vector<P> b)
    {
        ...
        { vector<P> p; for (P u:a) for (P v:b) p.push_back(plus(u,v)); return G::hull(p); }
        auto start=[](vector<P> &p)
        {
            auto it=min_element(p.begin(),p.end(),[](P u,P v) { return tie(u.y,u.x)<tie(v.y,v.x); });
            rotate(p.begin(),it,p.end());
        };
        start(a); start(b); int n=a.size(),m=b.size(),i=0,j=0; vector<P> result;
        while (i<n||j<m)
        {
...
struct IntegerGeometry3D
{
    ...
    struct Vector { I x,y,z; };
    static Vector diff(Point a,Point b) { return {I(a.x)-b.x,I(a.y)-b.y,I(a.z)-b.z}; }
    ...
    { auto v=cross(diff(b,a),diff(c,a)); return !v.x&&!v.y&&!v.z; }

...
struct MaxPlusMatrix
{
    ...
    int n; vector<vector<I>> a;
    MaxPlusMatrix(int n,bool identity=false):n(n),a(n,vector<I>(n,neg))
    { if (identity) for (int i=0;i<n;i++) a[i][i]=0; }
    MaxPlusMatrix operator*(const MaxPlusMatrix &b) const
    {
        assert(n==b.n); MaxPlusMatrix c(n);
        for (int i=0;i<n;i++) for (int k=0;k<n;k++) if (a[i][k]!=neg)
            for (int j=0;j<n;j++) if (b.a[k][j]!=neg)
...
struct LiChao
{
    ...
    vector<ll> xs; vector<Node> a{Node{}};
    LiChao(vector<ll> x):xs(x)
    { sort(xs.begin(),xs.end()); xs.erase(unique(xs.begin(),xs.end()),xs.end()); assert(!xs.empty()); }
    bool better(Line u,Line v,ll x) const
    { return v.id==-1||(u.id!=-1&&pair{u.value(x),u.id}<pair{v.value(x),v.id}); }
    void add(Line line) { assert(line.id>=0); add(0,0,(int)xs.size()-1,line); }
    void add(int p,int l,int r,Line line)
    {
        ...
        if (next==-1)
        {
            next=(int)a.size(); a.push_back(Node{});
            if (left) a[p].left=next; else a[p].right=next;
        }
        if (left) add(next,l,m,line); else add(next,m+1,r,line);
    }
    pair<I,int> query(ll x) const
    {
        int idx=lower_bound(xs.begin(),xs.end(),x)-xs.begin();
        assert(idx<(int)xs.size()&&xs[idx]==x);
        int p=0,l=0,r=(int)xs.size()-1; Line best;
        while (p!=-1)
        {
            if (better(a[p].line,best,x)) best=a[p].line;
            if (l==r) break;
            int m=(l+r)/2;
            if (idx<=m) { p=a[p].left; r=m; } else { p=a[p].right; l=m+1; }
        }
        assert(best.id!=-1); return {best.value(x),best.id};
    }
};
...
struct PersistentKth
{
    ...
    vector<Node> t{Node{}}; vector<int> root{0}; vector<long long> vals;
    PersistentKth(const vector<long long> &a):vals(a)
    {
        sort(vals.begin(),vals.end()); vals.erase(unique(vals.begin(),vals.end()),vals.end());
        for (long long x:a)
        { int p=lower_bound(vals.begin(),vals.end(),x)-vals.begin(); root.push_back(add(root.back(),0,(int)vals.size()-1,p)); }
    }
    int add(int old,int l,int r,int x)
    {
        int p=t.size(); Node copy=t[old]; t.push_back(copy); ++t[p].sum;
        if (l<r)
        {
            int m=(l+r)/2;
            if (x<=m) { int v=add(t[old].l,l,m,x); t[p].l=v; }
            else { int v=add(t[old].r,m+1,r,x); t[p].r=v; }
        }
        return p;
    }
    long long kth(int l,int r,int k) const
    {
        ...
        while (L<R)
        {
            ...
            if (k<=c) { u=t[u].l; v=t[v].l; R=m; }
            else { k-=c; u=t[u].r; v=t[v].r; L=m+1; }
        }
        return vals[L];
    }
};

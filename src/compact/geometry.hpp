...
struct IntegerGeometry
{
    ...
    static vector<Point> hull(vector<Point> p)
    {
        ...
        for (Point v:p)
        {
            while (h.size()>1&&cross(h[h.size()-2],h.back(),v)<=0) h.pop_back();
            h.push_back(v);
        }
        int low=(int)h.size();
        for (int i=n-2;i>=0;i--)
        {
            while ((int)h.size()>low&&cross(h[h.size()-2],h.back(),p[i])<=0) h.pop_back();
            h.push_back(p[i]);
        }
        h.pop_back(); return h; // CCW, no repeated endpoint, no collinear interior points.
    }
    static I area2(const vector<Point> &p)
    {
        I ans=0; int n=(int)p.size();
        for (int i=0;i<n;i++) ans+=I(p[i].x)*p[(i+1)%n].y-I(p[i].y)*p[(i+1)%n].x;
        return ans;
    }
    ...
    {
        int n=(int)p.size(); if (n<3) return contains(p,q);
        I a=cross(p[0],p[1],q),b=cross(p[0],p[n-1],q);
        if (a<0||b>0) return 0;
        if (!a) return on_segment(q,p[0],p[1])?1:0;
        if (!b) return on_segment(q,p[0],p[n-1])?1:0;
        int l=1,r=n-1;
        while (l+1<r) { int m=(l+r)/2; if (cross(p[0],p[m],q)>=0) l=m; else r=m; }
        I c=cross(p[l],p[r],q); return c<0?0:c==0?1:2;
    }
    static I diameter2(const vector<Point> &p)
    {
        ...
        {
            int k=(i+1)%n;
            while (cross(p[i],p[k],p[(j+1)%n])>cross(p[i],p[k],p[j])) j=(j+1)%n;
...
struct RealGeometry
{
    ...
    struct Circle { Point o; R r; };
    enum class Kind { none,one,two,infinite,degenerate };
    struct Result { Kind kind; vector<Point> p; };
    ...
    R segment_distance(Point p,Point a,Point b) const
    {
        ...
        R t=clamp(dot(p-a,v)/d,R(0),R(1)); return norm(p-(a+v*t));
    }
    Result line_intersection(Point a,Point b,Point c,Point d) const
    {
        Point u=b-a,v=d-c; R nu=norm(u),nv=norm(v);
        if (nu==0||nv==0) return {Kind::degenerate,{}};
        R det=cross(u,v);
        if (fabsl(det)<=eps*nu*nv)
            return {fabsl(cross(c-a,u))<=eps*nu*max(R(1),norm(c-a))?Kind::infinite:Kind::none,{}};
        return {Kind::one,{a+u*(cross(c-a,v)/det)}};
    }
    Result line_circle(Point a,Point b,Circle c) const
    {
        assert(c.r>=0); Point v=b-a; R len=norm(v);
        if (len==0) return {Kind::degenerate,{}};
        Point h=projection(c.o,a,b); R d=norm(h-c.o),tol=eps*max({R(1),d,c.r});
        if (d>c.r+tol) return {Kind::none,{}};
        if (fabsl(d-c.r)<=tol) return {Kind::one,{h}};
        R t=sqrtl(max(R(0),(c.r-d)*(c.r+d))); Point w=v*(t/len);
        return {Kind::two,{h-w,h+w}};
    }
    Result circles(Circle a,Circle b) const
    {
        assert(a.r>=0&&b.r>=0); Point v=b.o-a.o; R d=norm(v);
        R tol=eps*max({R(1),d,a.r,b.r});
        if (d<=tol)
        {
            if (fabsl(a.r-b.r)>tol) return {Kind::none,{}};
            if (a.r==0&&b.r==0) return {Kind::one,{a.o}};
        ...
        if (fabsl(d-(a.r+b.r))<=tol||fabsl(d-fabsl(a.r-b.r))<=tol)
            return {Kind::one,{h}};
        R y=sqrtl(max(R(0),(a.r-x)*(a.r+x))); Point w=perp(v)*(y/d);
        return {Kind::two,{h-w,h+w}};
    }
    R overlap(Circle a,Circle b) const
    {
        ...
        R x=acosl(clamp((d*d+a.r*a.r-b.r*b.r)/(2*d*a.r),R(-1),R(1)));
        R y=acosl(clamp((d*d+b.r*b.r-a.r*a.r)/(2*d*b.r),R(-1),R(1)));

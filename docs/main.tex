...
\lstset{language=C++,basicstyle=\ttfamily\scriptsize,keywordstyle=\bfseries,
...
\vspace{10mm}{\large 建设稿 · 2026-09-11\par}
\vfill
\begin{minipage}{0.82\textwidth}
\textbf{状态说明}\par
本稿包含首批已进行本地验证的模块与完整来源覆盖表的入口。
尚未完成 kuangbin、WIDA 全部条目的双码风迁移，尚未完成近三年中国赛站逐题覆盖审计，两套最大流已通过洛谷 P3376；其余模块尚无本项目 OJ AC 证据。
...
代码采用 C++20。动态版默认需要 \texttt{bits/stdc++.h} 和 \texttt{cassert}；
传统版各头文件列明依赖。抄写单个 struct 时需同时抄写其依赖结构。

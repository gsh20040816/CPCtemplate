...
对照 kuangbin 1.5.1 DA（书内页码从 11 页起）及 WIDA 的 jiangly SuffixArray/SA、打印稿字符串章节，将原有逐轮比较排序替换为稳定计数排序倍增。保留 `sa/rk/lcp`，增加整数字母表及重复初始化。整数序列为 0..alphabet-1；字节序列先按 unsigned char 转为 0..255。内部排名从 1 起、越界第二关键字为 0，外部无需哨兵，最终排名及后缀起点均为 0-based。WIDA 打印稿的 O(N) 说明与其倍增循环不符，此处按 O(n log n+alphabet) 记录。kuangbin 的 DC3、WIDA 03B 中额外的 RMQ/LCP/LCS 应用仍保留未完成标记。

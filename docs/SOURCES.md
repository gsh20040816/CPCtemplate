...
对照 WIDA 在线模板 `31 - 线性基（高斯消元法）.cpp` 与打印稿数据结构中的 LB，扩展 XorBasis/Xor_Basis。原有插入和最大异或接口保留；增加成员判断、非空子集最小值、不同异或结果的第 k 小、重建缓存和合并。WIDA jiangly 的 `Basis` 另带位置阈值，当前普通基不提供此语义，该条继续保留 pending。

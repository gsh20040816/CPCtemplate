...
测试同时验证任意点对的树上瓶颈等于穷举原图最小割，以及删掉每条树边后原图跨割容量等于该边权。包括四点三值容量全枚举、随机重边/自环、边顺序变化、单点空树、70 点高容量路径和零图。普通及 ASan/UBSan 均通过；传统头文件还单独实例化检查。通用树查询辅助函数和在线 AC 尚待补，不将参考章节直接标为全部完成。

...
StoerWagner / Global_Min_Cut 接收对称非负矩阵，返回最小割费用和原图中一侧的顶点。每轮按与已选集合的连接权最大者扩张，末尾顶点产生本轮割；合并最后两个顶点继续。group 始终维护收缩点代表的原始顶点，记录最优轮次时复制该列表，不能把收缩后的编号作为输出。矩阵实现为 O(n³)，不套用堆优化版本的复杂度。

输入必须至少两点；两侧都非空，并列结果不保证顺序或点数最小。非连通图自然返回 0，无需另建并查集。平行边先求和，对角线置零；负权不适用。每条无向边只计一次的总权严格小于 2^120，使中间收缩边权与连接权都不碰到初始哨兵。

tests/global_min_cut.cpp 穷举至四点图的每边 0/1/2 权，与全部二划分最小费用比较；同时对每次返回点集检查唯一性、两侧非空和原图跨割费用。另包含 500 个随机多重边累加矩阵、300 点环/完全图/零图及 2^100 权值缩放。普通与 ASan/UBSan 均通过，未取得该接口在线 AC。

## Gomory–Hu 树构建与割证书

两套 gomory_hu / Gomory_Hu 返回原顶点上的 n-1 条带权树边，覆盖零容量和不连通图。最小割对应路径最小边权，不是普通距离。每次最大流重新建立原图容量，不复用旧残量作为下一轮初始图；无向边建两条有向容量边，所以传统 Network_Flow 的 M 至少为无向输入边数的两倍。非自环边总容量须严格小于 LLONG_MAX，确保 flow 用默认上限求尽后才取残量可达点集。

初始 parent 全为 0，逐个顶点向父亲求最小割。对源侧所有符合 parent[v]==t 的其他顶点重挂，不能仅处理尚未轮到的顶点；后者未通过本库的树边割点集验证。若旧父亲的父亲落在源侧，还需交换相应父边位置并搬移旧边权。

测试同时验证任意点对的树上瓶颈等于穷举原图最小割，以及删掉每条树边后原图跨割容量等于该边权。包括四点三值容量全枚举、随机重边/自环、边顺序变化、单点空树、70 点高容量路径和零图。普通及 ASan/UBSan 均通过；传统头文件还单独实例化检查。树构建本身的证据不替代查询代码验证；后续查询验证见下一节，在线 AC 尚待补。

## 最小割树全点对查询

cut_tree_values / Cut_Tree_Values 从每个原顶点出发，递归 DFS 传递路径上的最小边权，得到 O(n²) 的查询表。只接受合法树：连通、无环、n-1 条边。零权树边正常参与最小值，不当作不存在。对角线 LLONG_MAX 是空路径哨兵，并不定义同点的最小割。Gomory-Hu 树可直接传入，普通非负加权树也可用于路径瓶颈查询。

tests/cut_tree_queries.cpp 使用独立 Floyd 最大瓶颈递推核对一千棵随机树，另检查 1200 点递归长链和 LLONG_MAX 边权。tests/gomory_hu.cpp 现将两套查询函数接入每棵生成树，再与原图所有二划分的点对最小割对比；返回的每条树边仍逐项验证原图割点集。普通及 ASan/UBSan 通过，WIDA 该节构建与全点对预处理均更新为本地验证。

## P4897 完整程序与点号转换

官方题面 https://www.luogu.com.cn/problem/P4897 中 n 表示最大点号，实际有 n+1 个顶点，编号 0..n。两套驱动均向构树与全点对查询函数传 n+1；传统版使用 static Network_Flow<501,3000>，每条无向边建立两条有向容量边，残量反边由底层模板另计。

tests/gomory_hu_application.py 以穷举所有割核验一百张小图的全部有序点对，并检查零权和零查询。最大用例为 501 顶点路径，每段三条平行边，共 1500 条输入边、十万条查询；独立路径区间最小值提供答案，含顶点 500 和相邻点查询。总计 103 张图，两套完整程序普通与 ASan/UBSan 均通过；尚无在线 AC。

## P4897 双版本在线记录

后续两套完整程序已通过全部 10 个测试点：动态版 https://www.luogu.com.cn/record/297591576 ，最长单点 252 ms、峰值 2.59 MB；传统版 https://www.luogu.com.cn/record/297592933 ，最长单点 185 ms、峰值 2.53 MB。当前题目时限为单点 500 ms。此 AC 覆盖构树与全点对查询组合的点对最小割值，不直接输出或判定树边割点集；后者及更大容量仍由本地测试分别支持。完整提交快照和 SHA256 见 verification/oj.json。

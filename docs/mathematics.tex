...
\section{幂的约数和：不除以 \texorpdfstring{$p-1$}{p-1}}\index{divisor sum}\index{geometric series}
...
初始取 $r=0,m=1$，依次合并 $x\equiv b\pmod n$。
...
本库当前 Fenwick<ModInt> 缺少复合赋值而不能编译，尚待补齐；这是类型组合缺口，不算算法测试通过。

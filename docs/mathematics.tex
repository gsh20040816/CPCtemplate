...
普通与 ASan/UBSan 均通过；区间加 1、后缀拼接和单点读取的应用已获 QOJ8240 双 AC，
其余通用接口范围仍由本地验证覆盖。
...
\clearpage
\section{杭州 2023 K：跨版本拼接应用}
\href{https://qoj.ac/problem/8240}{Card Game} 的完整驱动在
\texttt{verify/qoj/8240.*.cpp}，使用前节的可持久化区间模板。
...
以及稀疏重复大数组的短区间逐张模拟。
两套驱动均在线 AC，各 41 个正式测试和 3 个额外测试全过：
\href{https://qoj.ac/submission/2939235}{动态版}最慢 789 ms、452688 KB，
\href{https://qoj.ac/submission/2939291}{传统版}最慢 752 ms、452492 KB。
...
\href{https://www.luogu.com.cn/problem/P2921}{P2921}，均准备两套码风。
前两题检查跳转与可达步数，后一题仅使用“入环步数＋环长”，不能替代查询接口的验证。
线上评测尚待完成。

\clearpage
\section{杭州 2023 H：随机事件顺序与函数图}

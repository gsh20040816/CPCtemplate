...
lines=['# OI Wiki 分类映射', '', '固定官方导航来源：['+mapping['reference_commit'][:12]+']('+result['source_url']+')。', '', 'issue #7 指定的 oi-wiki.com 未能通过网页工具访问；此处明确采用 OI Wiki 官方仓库，不声称已核验该域名与官方导航相同。', '', '此表是分类准备与差异审计；PDF 尚未切换到此树。direct 表示直接知识点，application 表示应用，composite 表示一个代码条目横跨多个页面，related 表示仅有相关挂靠，需进一步处理。分类不代表上游整页内容已实现或验证。', '', '| 模板 | 上游完整层级 | 关系 | 处理说明 |', '| --- | --- | --- | --- |']
...
print(f'{len(leaves)} navigation pages; {len(catalog)} templates mapped; PDF migration remains separate')

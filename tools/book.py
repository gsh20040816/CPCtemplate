...
chapters={'tarjan':'Tarjan 与缩点','biconnected':'双连通分量与圆方树','vertex_removal':'删点连通性','edge_components':'边双缩点与边定向','bridge_augmentation':'桥树最少加边','data_structure':'数据结构','position_basis':'带位置线性基','basis_intersection':'线性空间求交','affine_segment_tree':'线段树常用修改','flow':'网络流','maximum_closure':'最大权闭合子图','graph':'图论','xor_walk':'图上异或行走','tree':'树上算法','offline_lca':'离线最近公共祖先','euler_lca':'在线最近公共祖先','lifting_lca':'倍增祖先与路径最值','centroid':'点分治','dsu_on_tree':'树上启发式合并','virtual_tree':'虚树','string':'字符串','suffix_lcp':'后缀查询与重复子串','palindromic_tree':'回文树','number_theory':'数论','exlucas':'合数模数组合数','modular_sqrt':'二次剩余','interpolation':'多项式插值','polynomial':'多项式','algebra':'代数与数论进阶','geometry':'计算几何','optimization':'优化与可持久化','persistent_array':'可持久化数组','tree_path_kth':'树上路径第 k 小','dynamic_kth':'动态区间顺序统计','persistent_distinct':'区间不同数统计','graph_advanced':'图论进阶','weighted_matching':'带权二分图匹配','geometry_extra':'精确几何进阶','support_hull':'凸包支撑点查询','dynamic_tree':'动态树','treap':'随机平衡树','splay':'伸展树','gcd_sequence':'状态序列维护','blossom':'一般图匹配','recurrence':'线性递推'}
body=[]

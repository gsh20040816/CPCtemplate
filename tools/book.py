...
for style in ['compact','classic']:
 body.append(r'\part{'+('紧凑动态风格' if style=='compact' else '传统接口风格')+'}')
 for file,title in chapters.items():
  ...
  for f,name,cn,info in cat:
   ...
   start=starts[idx][0];end=starts[idx+1][0] if idx+1<len(starts) else len(lines)
   # clang-format puts template declaration on its own preceding line.
   if start and lines[start-1].startswith('template'): start-=1
   if end and lines[end-1].startswith('template'): end-=1
   estimate=sum(max(1,(len(line.expandtabs(4))+109)//110) for line in lines[start:end])*8.5+70+(len(info)/65)*12
   if estimate<600: body.append(r'\Needspace{'+str(round(estimate))+'pt}')
   body.append(r'\section{'+cn+r'}\label{'+style+'-'+name+r'}\index{'+target.replace('_',r'\_')+'}')
   if style=='compact': body.append(esc(info))
   else:
    body.append('算法约束见紧凑版本第~\\pageref{compact-'+name+'}~页。本节代码独立可抄；采用下列实际接口名。')

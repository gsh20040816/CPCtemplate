 ...
 for file,title in chapters.items():
  ...
  for f,name,cn,info in cat:
   ...
   if name=='mixed_euler_orientation':
    ...
    estimate=(split-start)*10.2+70+(len(info)/65)*12
   if name=='release_bfs': body.append(r'\newpage')
   ...
   if style=='compact': body.append(esc(info))
    ...
    if name=='EulerLCA':body.append('声明 Euler\\_LCA<N,LOG>，Init(n) 清空树；Insert 加边，Build(root) 预处理，Lca 与 Distance 在线查询，Weighted\\_Distance 返回带权距离。LOG 层必须满足 2 的 LOG 次方大于 2n-1，默认 20 支持 50 万点；st 固定容量为 LOG 乘以 2N。大对象放全局或 static。')
   ...
   if name in ('DirectedEuler','UndirectedEuler') and style=='classic':
    ...
    second=next(i for i in range(first,end) if re.match(r'    int (advance|Advance)\(',lines[i]))
    body.append(r'\lstinputlisting[firstline='+str(start+1)+',lastline='+str(first)+']{../src/'+style+'/'+filename+'.hpp}')
    body.append(r'\newpage')
    body.append(r'\noindent 环、入环树与倍增表（接上页同一结构体与函数）：')
    body.append(r'\lstinputlisting[firstline='+str(first+1)+',lastline='+str(second)+',firstnumber='+str(first-start+1)+']{../src/'+style+'/'+filename+'.hpp}')
    body.append(r'\newpage')
    body.append(r'\noindent 跳转与最少步数接口（接上页同一结构体）：')
    body.append(r'\lstinputlisting[firstline='+str(second+1)+',lastline='+str(end)+',firstnumber='+str(second-start+1)+']{../src/'+style+'/'+filename+'.hpp}')
   elif name=='TreeDiameter' and style=='compact':
    ...
    first=next(i for i in range(start,end) if re.match(r'    void (dfs|Dfs)\(',lines[i]))
    second=next(i for i in range(first,end) if re.match(r'    int (lca|Lca)\(',lines[i]))
    body.append(r'\lstinputlisting[firstline='+str(start+1)+',lastline='+str(first)+']{../src/'+style+'/'+filename+'.hpp}')
    body.append(r'\newpage')
    ...
    first=next(i for i in range(start,end) if re.match(r'    int (change|Change)\(',lines[i]))
    second=next(i for i in range(first,end) if re.match(r'    ll (get|Get)\(',lines[i]))
    body.append(r'\lstinputlisting[firstline='+str(start+1)+',lastline='+str(first)+']{../src/'+style+'/'+filename+'.hpp}')
    body.append(r'\newpage')

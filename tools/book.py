 ...
 for file,title in chapters.items():
  ...
  for f,name,cn,info in cat:
   ...
   if name in ('DirectedEuler','UndirectedEuler') and style=='classic':
    ...
    body.append(r'\newpage')
   elif name in ('ExLucas','DynamicKth','SupportHull','SuffixLCP','PositionBasis','XorWalk','Lowlink','OfflineLCA','EulerLCA','DirectedEuler','UndirectedEuler','mixed_euler_orientation','odd_cycle_vertices'):
    if name=='odd_cycle_vertices':
     split=next(i for i in range(start,end) if 'edges(graph.blocks.size())' in lines[i])
    ...
    body.append(r'\lstinputlisting[firstline='+str(start+1)+',lastline='+str(split)+']{../src/'+style+'/'+filename+'.hpp}')
    if name in ('SupportHull','SuffixLCP','PositionBasis','XorWalk','Lowlink','OfflineLCA','EulerLCA','DirectedEuler','UndirectedEuler','mixed_euler_orientation','odd_cycle_vertices'):
     body.append(r'\newpage')
    else:

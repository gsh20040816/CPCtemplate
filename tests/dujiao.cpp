...
int main()
{
    ...
    for (int limit : {1,7,64,1000})
    {
        ...
        for (int x : queries)
        {
            assert(a.mertens(x) == pmu[x] && b.Mertens(x) == pmu[x]);
            assert(a.totient_sum(x) == pphi[x] && b.Totient_Sum(x) == pphi[x]);
        }
        assert(a.mmu == am && a.mphi == ap && b.mmu == bm && b.mphi == bp);
        assert(a.mmu.count(n) && a.mphi.count(n));
        assert(b.mmu.count(n) && b.mphi.count(n));
        a.mmu.clear();
        a.mphi.clear();
        assert(a.mertens(n) == pmu[n] && a.totient_sum(n) == pphi[n]);
    }
    // Cutoff independence at the documented upper bound: consistency, not a separate oracle.
    {
        DuJiao a(1000000);
        Du_Jiao b(2000000);
    ...
    cout << "DuJiao dual trial-prefix oracle, zero/cutoff boundaries, shuffled/repeated queries, cache rebuild and 1e11 cutoff consistency PASS\n";
}

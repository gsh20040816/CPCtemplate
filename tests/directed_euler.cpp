...
optional<vector<int>> brute(int n, const vector<pair<int, int>> &edges, int start)
{
    ...
    auto search = [&](auto &&self, int u, int mask) -> void
    {
        ...
        for (int i = 0; i < (int)edges.size(); i++)
            if (!(mask >> i & 1) && edges[i].first == u)
            {
                path.push_back(edges[i].second);
                self(self, edges[i].second, mask | (1 << i));
                path.pop_back();
            }
    };
    for (int u = 1; u <= n; u++)
...
int main()
{
    ...
    cout << "Directed Euler dual exhaustive trails, lexicographic optimum, edge certificates, multigraphs and 200000 recursive edges PASS\n";
}

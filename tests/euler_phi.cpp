...
int main()
{
    ...
    for (int n = 1; n <= 20000; n++)
    {
        ...
        {
            ull count = 0;
            for (int k = 1; k <= n; k++) count += gcd(k, n) == 1;
            assert(expected == count);
        }
        assert(euler_phi(n) == expected);
        assert(euler_phi(n, a) == expected);
    }
    vector<vector<ull>> lists = {{2, 2, 2, 3, 3, 5},
                                 vector<ull>(63, 2),
                                 vector<ull>(40, 3),
                                 {3, 5, 17, 257, 641, 65537, 6700417},
                                 {4294967291ULL, 4294967291ULL},
    ...
    for (auto fs : lists)
    {
        ...
        {
            ull p = fs[i];
            for (ull d = 2; d <= p / d; d++) assert(p % d);
            n *= p;
    ...
    cout << "Euler phi independent trial/rho paths, gcd/sieve oracles, repeated "
            "primes, uint64 composites and large prime PASS\n";
}
